...
def pagination(all,result,page,sort_order,user):
    ...
    #everything_l = [[i.book.book,i.book.author.author,i.book.publisher.publisher,i.book.year,i.limited_date,i.end_date,i.start_date,i.status] for i in everything]
    #print("----every---",everything_l)

    #borrow_book = ReservationModel.objects.filter(user=user,start_date__gt="2000-01-01")######
    borrow_book = ReservationModel.objects.select_related('book').filter(user=user,start_date__gt="2000-01-01")
    borrow_list = [i.book.id for i in borrow_book]

    result_list = [[i.id,i.images,i.book,i.author.author,i.publisher.publisher,i.year] for i in result]############  aaaaaaa
    reserved_list = [i.book.id for i in reserved_book]############  aaaaaaa
    ...
    t_book = ReservationModel.objects.select_related('book').filter(user=user,status="T")
    ...
    r_book = ReservationModel.objects.select_related('book').filter(user=user)
    ...
    book_list =  t_list + reserved_list2 + borrow_list2
